...
class FixedSplit extends StatefulWidget {
  /// Builds a split oriented along [axis].
  FixedSplit({
    required this.axis,
    required this.children,
    required this.initialChildrenSizes,
    Key? key,
    this.minSizes,
    this.splitters,
  })  : assert(children.length >= 2),
        assert(initialChildrenSizes.length >= 2),
        assert(initialChildrenSizes.length == children.length),
        super(key: key) {
    // Check that exactly one of the initialChildrenSizes is null.
    assert(initialChildrenSizes.where((size) => size == -1.0).length == 1);
  ...
  /// The initial childrenSize. Exactly one of the children index must be null.
  final List<double> initialChildrenSizes;
  ...
  @override
  State<StatefulWidget> createState() => _FixedSplitState();
}

class _FixedSplitState extends State<FixedSplit> {
  ...
  Widget _buildLayout(BuildContext context, BoxConstraints constraints) {
    ...
    // If `fractions` is empty, compute its initial value then rely on it.
    if (fractions.isEmpty) {
      final allocated = widget.initialChildrenSizes
              .where((size) => size != -1.0)
              .reduce((a, b) => a + b) /
          availableSize;
      fractions = List.from(widget.initialChildrenSizes.map((s) {
        if (s != -1.0) {
